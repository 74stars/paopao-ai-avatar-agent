...
- 可在线体验的静态预览。
- Electron + React + TypeScript + Three.js 的桌面 UI 壳。
- 能收发文本消息的飞书机器人骨架。
...
这次模糊不是 GitHub 把高分辨率图片二次压缩造成的。仓库中的以下六个文件使用了同一个 Git 对象：

- <code>assets/paopao-living-library.webp</code>
- <code>desktop-app/design/living-library-reference.webp</code>
- <code>desktop-app/public/assets/library-day.webp</code>
- <code>desktop-app/public/assets/library-night.webp</code>
- <code>preview/assets/library-day.webp</code>
- <code>preview/assets/library-night.webp</code>

它们的 SHA 一致，文件大小均为 15,064 bytes，像素尺寸为 900×563。当前 GitHub README 页面中，浏览器读取到的原图仍是 900×563，却渲染为约 838×524 CSS 像素；设备像素比为 2，意味着屏幕实际需要约 1676×1048 像素。桌面书房默认窗口为 1440×900，<code>background-size: cover</code> 又会把 900×563 放大约 1.6 倍。

仓库另有 <code>assets/paopao-bg.png</code>（1672×941，约 1.49 MB），但 README 和当前书房背景没有使用它，而且宽高比也不同，不能未经确认直接替换。真正的原始母版需要从本地重新导出。

另外，<code>preview/assets/paopao.webp</code> 约为 320×320、6,343 bytes。当前浏览器检查时它没有返回有效的 <code>naturalWidth</code>/<code>naturalHeight</code>，可能是 VP8X/ALPH WebP 的兼容或导出问题。必须在目标浏览器中做 decode 检查；如果仍失败，重新导出标准 WebP 或 PNG，不能只依赖文件扩展名。

### 9.2 永久资产规范

| 用途 | 最低尺寸 | 推荐格式 | 规则 |
|---|---:|---|---|
| 设计母版 | 3840×2400 | PNG/无损 WebP | sRGB；只读保存；不从小图放大 |
| Electron 2x 背景 | 2880×1800 | WebP quality 88–92 或 AVIF | 适配 1440×900 和 150% 缩放 |
| Electron 3x/高端屏 | 3840×2400 | WebP quality 88–92 | 保留细节，按需加载 |
| README 展示图 | 宽度至少 1920 | WebP/PNG | 页面显示宽度不超过 960 |
| 预览网页 | 1280、2560 两档 | WebP/AVIF | 使用 <code>image-set</code> 或 <code>srcset</code> |
| 列表缩略图 | 320、640 两档 | WebP | 只用于列表，不替换原图 |

当前 900px 文件不能通过再次导出“变清晰”，必须回到本地原始设计文件。

### 9.3 发布实现

网页背景使用两档资源，避免 CSS 放大低分辨率文件：

~~~css
.library-room {
  background-image: image-set(
    url("./assets/library-day-1280.webp") 1x,
    url("./assets/library-day-2560.webp") 2x
  );
...
README 使用一张至少 1920px 宽的展示图，并限制显示宽度；不要把聊天截图、剪贴板截图或低清预览图当成正式资产上传。

### 9.4 上传和校验流程

1. 本地保存母版和导出参数，不经过聊天软件或截图流程。
2. 导出 day/night 两张不同的成品，不复用同一文件名和内容。
3. 上传前记录 SHA-256、像素宽高、色彩空间和文件大小。
4. GitHub 上传后通过 API 或 Raw URL 下载，再次校验 SHA-256 和像素尺寸。
5. Pages 发布后再下载线上文件校验；GitHub Pages 应原样提供文件。
6. 在 1366×768、1440×900、1920×1080 和 125%/150% 缩放下截图验收。

### 9.5 CI 图片质量门禁

CI 至少检查：

- 背景宽度不低于 2880，高度不低于 1800。
- 宽高比与设计基准一致，允许误差不超过 1%。
- day/night SHA-256 不相同。
- 透明 Logo/泡泡图片在 Chromium 和 Windows WebView 中都能 decode，<code>naturalWidth</code> 和 <code>naturalHeight</code> 大于 0。
- 生成文件不是 Git LFS 指针或 HTML 错误页。
- 发布包和 Pages 中的资源都能成功加载。
- 视觉回归截图没有明显马赛克、白屏、错误裁切或主题错配。

如果将来使用自动感知质量工具，可以把母版缩放后的 SSIM 作为警告指标；尺寸、哈希和线上加载失败应作为硬门禁。
...
| 阶段 | 时间 | 必须交付 |
|---|---:|---|
| M0 基线 | 3–5 天 | 锁文件、统一 README、修复资源路径、CI、高清底图、迁移框架 |
...
- [ ] P0-02：从本地母版重新导出高清 day/night 背景，替换所有 900×563 资产。
...
12. 1440×900 和 150% 缩放下背景无明显马赛克；day/night 资源不同且都能加载。
...
解决：母版和发布图分离、尺寸/哈希硬门禁、线上下载校验和多分辨率截图验收。

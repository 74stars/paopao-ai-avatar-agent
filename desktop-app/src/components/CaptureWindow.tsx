...
export function CaptureWindow() {
  ...
  useEffect(() => window.paopao?.onDomainEvent((event) => {
    ...
    if (["queued", "validating", "quiescing", "replacing", "reopening"].includes(event.status)) {
      setRestoring(true);
      setMessage("正在恢复备份，暂时无法记录。");
    } else if (event.status === "succeeded") {
      setRestoring(false);
      setMessage("备份恢复完成。");
    } else {
      setRestoring(false);
      setMessage("备份恢复失败，请在活书房中查看恢复记录。");
    }
  }), []);
  ...
  async function save() {
    ...
    try {
      ...
      setMessage(result.data.deduplicated ? "已保存。" : "已保存，正在整理。");
    } catch {
      setMessage("保存失败，输入内容仍保留，请稍后重试。");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <section className="capture-window" data-testid="capture-window">
      <header>
        <strong>泡泡</strong>
        <button className="icon-command" type="button" aria-label="关闭" title="关闭" onClick={() => window.paopao?.windows.hideCapture()}>×</button>
      </header>

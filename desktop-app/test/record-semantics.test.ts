...
import { EntryGovernance } from "../src/components/EntryGovernance.js";
...
describe("record semantics", () => {
  it("shows one record concept and reveals the initial version only after an edit", () => {
    const initial = renderToStaticMarkup(createElement(RecordContent, { detail: baseDetail, onUpdated: async () => undefined }));
    expect(initial).toContain("记录内容");
    expect(initial).toContain("编辑记录");
    expect(initial).not.toContain("最初记录</summary>");
    ...
    expect(edited).toContain("最初记录</summary>");
  });

  it("offers field-level AI adjustments without derivation or JSON terminology", () => {
    ...
    expect(markup).toContain("调整 AI 整理");
    expect(markup).toContain("保存分类");
    expect(markup).toContain("AI 摘要");
    expect(markup).toContain("保存摘要");
